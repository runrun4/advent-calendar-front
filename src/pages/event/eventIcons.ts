 ...
 * イベントアイコンの定義。
 *
 * EventNameField.tsx から切り出した。コンポーネント以外の輸出を混ぜると
 * react-refresh/only-export-components に触れて Fast Refresh が効かなくなるため。
 */
import {
  CalendarDays,
  Music,
 ...
 * id からアイコンを引くための表。
 *
 * 関数呼び出しで返すと react-hooks/static-components が
 * 「レンダー中にコンポーネントを生成している」と見なすため、
 * 参照側ではこの表を直接引く。
 */
...
export const DEFAULT_EVENT_ICON: LucideIcon = CalendarDays

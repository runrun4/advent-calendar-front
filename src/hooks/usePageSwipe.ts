...
export function usePageSwipe<T extends string>(tabs: T[], initialTab: T) {
  ...
  const onPointerMove = (event: PointerEvent<HTMLElement>) => {
    ...
    if (gestureAxis.current === 'pending') {
      ...
      gestureAxis.current = isHorizontal ? 'horizontal' : 'vertical'
    ...
    const atEnd = currentIndex === tabs.length - 1 && deltaX < 0
  ...
  const onPointerUp = (event: PointerEvent<HTMLElement>) => {
    ...
    if (axis !== 'horizontal' || Math.abs(deltaX) < SWIPE_THRESHOLD) {
      setDragOffset(0)
      return
    }
